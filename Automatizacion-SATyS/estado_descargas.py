...
def _leer_metadata_internos(carpeta: Path) -> dict | None:
    """Lee metadata_completo.json de Internos; None significa evidencia no utilizable."""
...
def _carpeta_internos_descarga_completa(carpeta: Path, bandeja: str, folio: str) -> bool:
    """Valida solamente la descarga: metadata, conteo y archivos físicos auditados."""
    metadata = _leer_metadata_internos(carpeta)
    if metadata is None:
        return False
...
def _postproceso_internos_completo(carpeta: Path) -> bool:
    """Valida Excel/organización cuando la metadata nueva declara ese estado.

    La ausencia de ``postproceso_internos`` también se considera pendiente.
    Esto hace una migración segura de descargas de versiones anteriores: si la
    descarga física ya está completa, Parte 1 NO la vuelve a bajar, pero el
    objetivo sí pasa por Excel/organización una vez y queda sellado con la nueva
    marca. Así también se recuperan correctamente corridas interrumpidas antes
    de Parte 4.
    """
    metadata = _leer_metadata_internos(carpeta)
    if metadata is None:
        return False
    post = metadata.get("postproceso_internos")
    if post is None:
        # Migración deliberadamente estricta sólo para Internos. La descarga
        # puede estar completa, pero falta evidencia de que Excel/organización
        # finalizaron; por tanto el objetivo debe pasar por postproceso.
        return False
    if not isinstance(post, dict):
        return False
    return (
        post.get("ok") is True
        and post.get("excel_ok") is True
        and post.get("organizado_ok") is True
    )


def _carpeta_internos_completa(carpeta: Path, bandeja: str, folio: str) -> bool:
    """True sólo si descarga y postproceso declarado quedaron completos."""
    return (
        _carpeta_internos_descarga_completa(carpeta, bandeja, folio)
        and _postproceso_internos_completo(carpeta)
    )


def _candidatos_carpetas_internos(descargas_base: Path, bandeja: str, folio: str) -> list[Path]:
...
def objetivo_internos_descarga_completa(
    descargas_base: Path,
    bandeja: str,
    folio: str,
) -> bool:
    """True cuando (bandeja, folio) conserva toda la descarga auditada.

    No exige que Excel/organización hayan terminado; se usa dentro de Parte 1
    para decidir qué folios necesitan volver a descargarse.
    """
    vistos: set[str] = set()
    for carpeta in _candidatos_carpetas_internos(descargas_base, bandeja, folio):
        clave = str(carpeta).casefold()
        if clave in vistos or not carpeta.is_dir():
            continue
        vistos.add(clave)
        if _carpeta_internos_descarga_completa(carpeta, bandeja, str(folio).strip()):
            return True
    return False
...
def objetivo_internos_esta_completo(
    ...
    """True si (bandeja, folio) terminó descarga + Excel + organización declarada."""
    folio_limpio = str(folio or "").strip()
    vistos: set[str] = set()
    for carpeta in _candidatos_carpetas_internos(descargas_base, bandeja, folio_limpio):
        clave = str(carpeta).casefold()
        if clave in vistos or not carpeta.is_dir():
            continue
        vistos.add(clave)
