# 2026.08.21-portable-oci-api-v1-8082-folio-modal-watchdog2

- Integra el soporte de la ventana morada `Archivo PDF` de `folio-modal1` con el watchdog de inactividad de Internos.
- Cada segmento de Internos corre en un proceso hijo aislado; si deja de producir actividad durante `SATYS_INTERNOS_WORKER_IDLE_TIMEOUT` (900 s por defecto), sólo ese árbol Chromium se termina y el resto continúa.
- Después de cada intento audita `metadata_completo.json` y los archivos físicos y reintenta únicamente los `(bandeja, folio)` que siguen incompletos.
- Conserva la marca `postproceso_internos`: una descarga puede estar completa pero seguir pendiente hasta finalizar Excel y organización.
- El smoke de seis bandejas vuelve a ejecutarse en el proceso padre para que su inspección sea realmente sin descargas, aun cuando la producción use workers aislados.
- Acota `scroll_into_view_if_needed` y el screenshot de diagnóstico para que una página Playwright degradada no pueda bloquear indefinidamente el manejo de errores.
