...
def actualizar_postproceso_internos(
    carpeta: Path,
    resultado: dict | None = None,
    *,
    error: str = "",
) -> bool:
    """Persiste el estado Excel/organización en metadata_completo.json.

    Parte 1 marca todo objetivo Internos nuevo/reprocesado como PENDIENTE. Esta
    función sólo lo convierte a OK cuando Excel y organización terminaron. Así,
    si el proceso se corta después de descargar pero antes de Parte 4, la
    siguiente corrida diaria vuelve a seleccionar ese (bandeja, folio).
    """
    path = Path(carpeta) / "metadata_completo.json"
    try:
        data = json.loads(path.read_text(encoding="utf-8-sig"))
        if not isinstance(data, dict):
            raise ValueError("metadata_completo.json no contiene un objeto")
        ...
        if resultado is None:
            ...
            excel_ok = resultado.get("excel_ok") is True
            organizado_ok = (
                True if not ORGANIZAR_DESCARGAS
                else resultado.get("organizado_ok") is True
            )
            ok = excel_ok and organizado_ok
            estado = "OK" if ok else "ERROR"
            output_dir = str(resultado.get("output_dir") or "")
        ...
        data["postproceso_internos"] = {
            ...
            "organizacion_requerida": bool(ORGANIZAR_DESCARGAS),
        ...
        log.error("❌ No se pudo actualizar postproceso Internos en %s: %s", carpeta, exc)
        return False
...
def ejecutar_descarga_internos(
    ...
    """Ejecuta Parte 1 sólo para objetivos cuya descarga siga incompleta.

    Un objetivo puede estar pendiente por Excel/organización aunque sus anexos
    ya estén íntegros. En ese caso no se vuelve a golpear SATyS: se salta Parte 1
    y continúa directamente con Partes 3-4.
    """
    try:
        import Parte1_descarga
    except ImportError as e:
    ...
    objetivos_temporal: Path | None = None
    objetivos_efectivos = objetivos_path
    if objetivos_path:
        try:
            objetivos = cargar_objetivos_internos(objetivos_path)
            pendientes_descarga = [
                item for item in objetivos
                if not objetivo_internos_descarga_completa(
                    DESCARGA_BASE, item["bandeja"], item["folio"]
                )
            ]
        except Exception as exc:
            log.error("❌ No se pudo preparar la auditoría previa de Internos: %s", exc)
            return 1

        if not pendientes_descarga:
            log.info(
                "✅ [PARTE 1] Los %d objetivo(s) tienen descarga completa; "
                "se continuará con Excel/organización sin volver a descargar.",
                len(objetivos),
            )
            return 0
        if len(pendientes_descarga) != len(objetivos):
            log.info(
                "♻️  [PARTE 1] %d/%d objetivo(s) requieren descarga; los demás "
                "continuarán sólo en postproceso.",
                len(pendientes_descarga), len(objetivos),
            )
        fd, nombre_tmp = tempfile.mkstemp(prefix="satys_internos_descarga_", suffix=".json")
        os.close(fd)
        objetivos_temporal = Path(nombre_tmp)
        objetivos_temporal.write_text(
            json.dumps({"objetivos": pendientes_descarga}, ensure_ascii=False, indent=2),
            encoding="utf-8",
        )
        objetivos_efectivos = objetivos_temporal
    ...
    try:
        ...
        if objetivos_efectivos:
            sys.argv += ["--internos-objetivos", str(objetivos_efectivos)]
        ...
        sys.argv = original_argv
        if objetivos_temporal is not None:
            objetivos_temporal.unlink(missing_ok=True)
...
def procesar_folio(
    ...
    if ORGANIZAR_DESCARGAS:
        if rpc_resultado and rpc_resultado.get("ok"):
            ...
            resultado["organizado_ok"] = bool(archivos_copiados)
        ...
        if args.internos:
            ...
            if claves_objetivo_i:
                ...
                for candidato in carpetas_internos:
                    ...
                    if clave in claves_objetivo_i and objetivo_internos_descarga_completa(
                        DESCARGA_BASE,
                        str(meta_candidato.get("bandeja_internos") or "").strip(),
                        folio_tabla,
                    ):
                ...
                if faltantes_i:
                    ...
                    log.error(
                        "❌ %d objetivo(s) Internos no tienen descarga local completa: %s",
                        len(faltantes_i),
            ...
            for idx_i, (carpeta_int, folio_id_int, registro_ref_int) in enumerate(carpetas_internos, 1):
                ...
                actualizar_postproceso_internos(carpeta_int)
                try:
                    ...
                    log.exception(
                        "❌ Error inesperado en postproceso Internos %s/%s; se continuará con los demás.",
                        folio_id_int,
                        folio_para_excel,
                    )
                    resultado_i = {
                        "folio": folio_para_excel,
                        "folio_id": folio_id_int,
                        "descargas_dir": str(carpeta_int),
                        "modo_internos": True,
                        "excel_ok": False,
                        "organizado_ok": False,
                        "error": str(exc_i),
                    }
                post_ok_i = actualizar_postproceso_internos(
                    carpeta_int,
                    resultado_i,
                    error=str(resultado_i.get("error") or ""),
                )
                resultado_i["postproceso_ok"] = post_ok_i
                if not post_ok_i:
                    rc_descarga_internos = rc_descarga_internos or 1
