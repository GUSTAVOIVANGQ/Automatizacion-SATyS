...
from estado_descargas import (
    objetivo_internos_descarga_completa,
...
INTERNOS_WORKER_IDLE_TIMEOUT = max(
    60,
    int(os.getenv("SATYS_INTERNOS_WORKER_IDLE_TIMEOUT", str(max(900, TIMEOUT_REGISTRO)))),
)
INTERNOS_WORKER_POLL_SECONDS = max(
    0.5,
    float(os.getenv("SATYS_INTERNOS_WORKER_POLL_SECONDS", "2")),
)
...
def screenshot(page, nombre: str):
    ...
    try:
        # Una página Chromium dañada no debe poder congelar el manejo de error.
        # El screenshot es evidencia auxiliar; nunca es parte crítica del flujo.
        ...
        while pagina_doc < max_paginas_doc:
            ...
            for i in range(filas_count):
                ...
                if not descargado:
                    url_popup = ""
                    try:
                        # SATyS puede dejar un botón re-renderizándose mientras el
                        # spinner desaparece. Un fallo de scroll debe afectar sólo
                        # este documento, no abortar los documentos restantes del
                        # folio ni dejar el worker detenido indefinidamente.
                        boton_ver_doc.scroll_into_view_if_needed(
                            timeout=min(15_000, TIMEOUT_NAV)
                        )
...
def _ejecutar_worker_internos_aislado(
    bandeja: str,
    folios_json: Path,
    resultado_json: Path,
) -> int:
    """Entrada del proceso hijo matable para un segmento de Internos."""
    started = datetime.now().isoformat()
    try:
        raw = json.loads(folios_json.read_text(encoding="utf-8-sig"))
        folios = [
            str(item).strip()
            for item in (raw if isinstance(raw, list) else [])
            if re.fullmatch(r"\d{1,15}", str(item).strip())
        ]
        _nombre, resultados = _worker_bandeja_internos(bandeja, folios or None)
        completos = [
            folio for folio in folios
            if objetivo_internos_descarga_completa(DESCARGA_BASE, bandeja, folio)
        ]
        payload = {
            "ok": (not folios) or len(completos) == len(folios),
            "bandeja": bandeja,
            "folios": folios,
            "folios_completos": completos,
            "started_at": started,
        ...
        log.exception("[INT-WORKER-PROC] Error fatal en bandeja '%s'.", bandeja)
        payload = {
            "ok": False,
            "bandeja": bandeja,
            "started_at": started,
...
def _leer_resultado_worker_internos(
    path: Path,
    bandeja: str,
    rc: int,
    worker_log: Path,
    ...
    if path.exists():
        try:
            ...
            return _resultado_error_bandeja_internos(
                bandeja,
                "ERROR_RESULTADO_WORKER_INTERNOS",
                f"No se pudo leer JSON del worker: {exc}. Log: {worker_log}",
            )
    return _resultado_error_bandeja_internos(
        bandeja,
        "ERROR_WORKER_SIN_RESULTADO_INTERNOS",
        f"Worker terminó sin JSON. return_code={rc}. Log: {worker_log}",
    )


def _ejecutar_worker_bandeja_internos_watchdog(
    bandeja: str,
    folios_objetivo: list[str] | None,
) -> tuple[str, list]:
    """Ejecuta un segmento en proceso hijo y mata sólo ese árbol si queda inactivo.

    El límite es por *inactividad de log*, no por duración total. Un segmento
    puede tardar muchas horas siempre que siga avanzando. Esto evita repetir el
    congelamiento observado cuando Chromium/Playwright dejó un worker vivo sin
    producir más salida.
    """
    folios = [str(f).strip() for f in (folios_objetivo or []) if str(f).strip()]
    worker_dir = Path("logs") / "workers_internos"
    worker_dir.mkdir(parents=True, exist_ok=True)
    token = uuid.uuid4().hex[:10]
    slug = _slug_internos(bandeja)
    worker_log = worker_dir / f"{datetime.now():%Y%m%d_%H%M%S}_{slug}_{token}.worker.log"
    resultado_json = Path(tempfile.gettempdir()) / f"satys_int_result_{token}.json"
    folios_json = Path(tempfile.gettempdir()) / f"satys_int_folios_{token}.json"
    folios_json.write_text(json.dumps(folios, ensure_ascii=False), encoding="utf-8")

    cmd = [
        sys.executable,
        str(Path(__file__).resolve()),
        "--headless" if HEADLESS else "--visible",
        "--_internos-worker-bandeja", bandeja,
        "--_internos-worker-folios-json", str(folios_json),
        "--_resultado-json", str(resultado_json),
    ]
    env = os.environ.copy()
    ...
    info = {"worker_log": worker_log, "log_pos": 0}
    proc = None
    timed_out = False
    rc = 1
    try:
        with worker_log.open("w", encoding="utf-8", errors="replace") as fh:
            ...
            ultima_actividad = inicio
            ultimo_tamano = -1
            while proc.poll() is None:
                ahora = time.monotonic()
                try:
                    tamano = worker_log.stat().st_size
                except OSError:
                    tamano = ultimo_tamano
                if tamano != ultimo_tamano:
                    ultimo_tamano = tamano
                    ultima_actividad = ahora
                    _relay_worker_log(info, f"INT:{bandeja}")
                if ahora - ultima_actividad > INTERNOS_WORKER_IDLE_TIMEOUT:
                    timed_out = True
                    _kill_process_tree(
                        proc,
                        f"Internos/{bandeja}",
                        motivo=f"INACTIVO>{INTERNOS_WORKER_IDLE_TIMEOUT}s",
                    )
                    break
                time.sleep(INTERNOS_WORKER_POLL_SECONDS)
            if proc.poll() is None:
                try:
                    proc.wait(timeout=10)
                except Exception:
                    _kill_process_tree(proc, f"Internos/{bandeja}", motivo="cierre_watchdog")
            rc = int(proc.poll() if proc.poll() is not None else 1)
        _relay_worker_log(info, f"INT:{bandeja}", final=True)
        if timed_out:
            return bandeja, _resultado_error_bandeja_internos(
                bandeja,
                "ERROR_WORKER_TIMEOUT_INACTIVO",
                (
                    f"Segmento sin actividad durante {INTERNOS_WORKER_IDLE_TIMEOUT}s; "
                    f"se terminó su proceso y Chromium. Log: {worker_log}"
                ),
            )
        return bandeja, _leer_resultado_worker_internos(
            resultado_json,
            bandeja,
            rc,
            worker_log,
        ...
        if proc is not None and proc.poll() is None:
            _kill_process_tree(proc, f"Internos/{bandeja}", motivo="excepcion_watchdog")
        return bandeja, _resultado_error_bandeja_internos(
            bandeja,
            "ERROR_WATCHDOG_INTERNOS",
            str(exc),
        )
    finally:
        resultado_json.unlink(missing_ok=True)
        folios_json.unlink(missing_ok=True)
...
def _worker_bandeja_internos_con_reintentos(
    ...
    """Reintenta sólo los Folios Internos que sigan con descarga incompleta.

    Cada intento corre en un proceso hijo vigilado. Si un Chromium queda sin
    actividad, el watchdog mata únicamente ese segmento. Después de cada intento
    se consulta la evidencia real de ``metadata_completo.json`` + archivos y el
    siguiente intento recibe únicamente los folios todavía incompletos.
    """
    intentos_totales = INTERNOS_WORKER_REINTENTOS + 1

    # Smoke/recorrido sin objetivos explícitos conserva la semántica histórica:
    # reintento por error de infraestructura o segmento vacío.
    if not folios_objetivo:
        ultimo_resultado: list = []
        smoke_in_process = os.getenv("SATYS_INTERNOS_SMOKE_IN_PROCESS", "0") == "1"
        for intento in range(1, intentos_totales + 1):
            if smoke_in_process:
                nombre_bandeja, ultimo_resultado = _worker_bandeja_internos(bandeja, None)
            else:
                nombre_bandeja, ultimo_resultado = _ejecutar_worker_bandeja_internos_watchdog(
                    bandeja,
                    None,
                )
            codigos = {
            ...
            if ultimo_resultado and not fallo_infraestructura:
                return nombre_bandeja, ultimo_resultado
            if intento < intentos_totales:
                motivo = ", ".join(sorted(codigos)) or "segmento vacio"
                log.warning(
                    "[INT-RETRY] Bandeja '%s' intento %d/%d no valido (%s); reintentando.",
                    bandeja,
                    intento,
                    intentos_totales,
                    motivo,
        ...
        return bandeja, ultimo_resultado or _resultado_error_bandeja_internos(
            bandeja,
            "ERROR_SEGMENTO_INTERNO_VACIO",
            f"La bandeja terminó vacía tras {intentos_totales} intento(s).",
        )

    originales = list(dict.fromkeys(
        str(folio).strip()
        for folio in folios_objetivo
        if re.fullmatch(r"\d{1,15}", str(folio).strip())
    ))
    pendientes = [
        folio for folio in originales
        if not objetivo_internos_descarga_completa(DESCARGA_BASE, bandeja, folio)
    ]
    if not pendientes:
        log.info(
            "[INT-RETRY] Bandeja '%s': los %d objetivo(s) ya tienen descarga auditada; se omite worker.",
            bandeja,
            len(originales),
        )
        return bandeja, []

    resultados_por_folio: dict[str, list[dict]] = {}
    errores_generales: list[dict] = []

    for intento in range(1, intentos_totales + 1):
        objetivo_intento = list(pendientes)
        nombre_bandeja, resultados = _ejecutar_worker_bandeja_internos_watchdog(
            bandeja,
            objetivo_intento,
        )

        # Sustituye la evidencia del intento anterior sólo para los Folios que
        # realmente se volvieron a intentar; errores antiguos no deben hacer que
        # una recuperación posterior termine con return code falso.
        for folio in objetivo_intento:
            resultados_por_folio[folio] = []
        for item in resultados:
            if not isinstance(item, dict):
                continue
            folio_item = str(item.get("folio") or "").strip()
            if folio_item in resultados_por_folio:
                resultados_por_folio[folio_item].append(item)
            else:
                errores_generales.append(item)

        pendientes = [
            folio for folio in objetivo_intento
            if not objetivo_internos_descarga_completa(DESCARGA_BASE, bandeja, folio)
        ]
        recuperados = len(objetivo_intento) - len(pendientes)
        log.info(
            "[INT-AUDIT] Bandeja '%s' intento %d/%d: %d recuperado(s), %d pendiente(s).",
            bandeja,
            intento,
            intentos_totales,
            recuperados,
        ...
        if intento < intentos_totales:
            log.warning(
                "[INT-RETRY] Bandeja '%s': se reintentaran sólo %d Folio(s) todavía incompletos.",
                bandeja,
                len(pendientes),
            )
            if INTERNOS_WORKER_ESPERA:
    ...
    salida: list[dict] = []
    for folio in originales:
        salida.extend(resultados_por_folio.get(folio, []))

    # Si un folio continuó incompleto pero el worker no dejó un resultado
    # específico (por ejemplo fue terminado por watchdog), crear evidencia
    # controlada para el reporte sin perder la condición de reproceso futuro.
    for folio in pendientes:
        if not resultados_por_folio.get(folio):
            salida.append({
                "folio": folio,
                "archivo": f"ERROR_INTERNO_INCOMPLETO_{folio}",
                "tipo": "ERROR",
                "ruta": "",
                "tamano_kb": 0,
                "ok": False,
                ...
                "registro": "",
                "carpeta": "",
                "error": "El objetivo sigue incompleto después de los reintentos automáticos.",
            })

    # Con objetivos explícitos sólo retenemos errores generales si todavía hay
    # pendientes. Si todos se recuperaron, el estado físico auditado es la fuente
    # de verdad y un timeout transitorio de un intento previo no debe contaminar
    # el resultado final.
    if pendientes:
        salida.extend(errores_generales)
    return bandeja, salida
...
def guardar_metadata_completo(
    ...
    # Sólo Internos participa en la garantía de reproceso extremo a extremo.
    # Una descarga nueva/reprocesada queda PENDIENTE hasta que main_procesar
    # confirme que también terminó Excel + organización. Si el proceso cae
    # entre Parte 1 y Parte 4, la siguiente corrida volverá a seleccionar este
    # (bandeja, folio) aunque sus documentos ya estén completos.
    if fuente == "INTERNOS_DOCUMENTOS_ANEXOS":
        data["postproceso_internos"] = {
            "ok": False,
            "estado": "PENDIENTE",
            "excel_ok": False,
            "organizado_ok": False,
...
def main():
    ...
    parser.add_argument("--_internos-worker-bandeja", type=str, default="", help=argparse.SUPPRESS)
    parser.add_argument("--_internos-worker-folios-json", type=str, default="", help=argparse.SUPPRESS)
    ...
    if args._internos_worker_bandeja:
        if not args._internos_worker_folios_json:
            log.error("[INT-WORKER-PROC] Falta --_internos-worker-folios-json")
            return 2
        resultado_path = (
            Path(args._resultado_json)
            if args._resultado_json
            else Path(tempfile.gettempdir()) / f"satys_int_result_{uuid.uuid4().hex}.json"
        )
        return _ejecutar_worker_internos_aislado(
            args._internos_worker_bandeja.strip(),
            Path(args._internos_worker_folios_json),
            resultado_path,
        )
    ...
    if args.internos:
        ...
        if objetivos_int:
            pendientes_finales = [
                item for item in objetivos_int
                if not objetivo_internos_descarga_completa(
                    DESCARGA_BASE,
                    item["bandeja"],
                    item["folio"],
                )
            ...
            if pendientes_finales:
                log.error(
                    "[INT-AUDIT] %d objetivo(s) continúan con descarga incompleta tras los reintentos.",
                    len(pendientes_finales),
                )
                return 2
            log.info(
                "[INT-AUDIT] Los %d objetivo(s) solicitados conservan descarga completa auditada.",
                len(objetivos_int),
            )
            return 0
