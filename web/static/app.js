...
const els = {
  ...
  repairPill: $('repair-pill'), repairStatus: $('repair-status'), repairCurrent: $('repair-current'), repairStarted: $('repair-started'),
  repairDetected: $('repair-detected'), repairProcessed: $('repair-processed'), repairResolved: $('repair-resolved'), repairUnresolved: $('repair-unresolved'), repairPending: $('repair-pending'),
  repairRetries: $('repair-retries'), repairUpdateOutputs: $('repair-update-outputs'), repairRedownloadFiles: $('repair-redownload-files'), repairResumeBtn: $('repair-resume-btn'), repairNewBtn: $('repair-new-btn'), repairStopBtn: $('repair-stop-btn'), repairMessage: $('repair-message'),
  repairLogSource: $('repair-log-source'), repairLogBox: $('repair-log-box'), repairAutoscroll: $('repair-autoscroll'),
...
const viewTitles = {
  ...
  reparacion: ['SATyS — Recuperación y continuidad', 'Reparar id_solicitante'],
...
function renderRepair(data) {
  if (!els.repairPill) return;
  const running = data.running === true;
  if (running) setPill(els.repairPill, 'running', data.status === 'stopping' ? 'Deteniendo' : 'En ejecución');
  else if (data.ok === true) setPill(els.repairPill, 'ok', 'Finalizado');
  else if (data.status === 'completed_with_warnings') setPill(els.repairPill, 'warn', 'Con pendientes');
  else if (data.ok === false || ['failed', 'interrupted', 'blocked'].includes(data.status)) setPill(els.repairPill, 'error', 'Interrumpido');
  else setPill(els.repairPill, 'neutral', 'Sin iniciar');

  const summary = data.summary || {};
  setText(els.repairStatus, data.status || 'not_started');
  setText(els.repairCurrent, data.current || '—');
  setText(els.repairStarted, data.started_at || '—');
  setText(els.repairDetected, summary.detected ?? 0);
  setText(els.repairProcessed, summary.processed ?? 0);
  setText(els.repairResolved, summary.resolved ?? 0);
  setText(els.repairUnresolved, summary.unresolved ?? 0);
  setText(els.repairPending, summary.pending ?? 0);
  setText(els.repairMessage, data.mensaje || 'Checkpoint listo.');
  els.repairResumeBtn.disabled = running;
  els.repairNewBtn.disabled = running;
  els.repairStopBtn.disabled = !running;
}
...
async function startRepair(reiniciarCola = false) {
  const action = reiniciarCola ? 'crear una cola nueva y descartar el checkpoint anterior' : 'iniciar o reanudar desde el checkpoint';
  if (!confirm(`¿Deseas ${action}?`)) return;
  ...
  try {
    const result = await fetchJson('/api/reparacion-id/iniciar', {
      ...
      headers: {'Content-Type': 'application/json'},
      body: JSON.stringify({
        reiniciar_cola: reiniciarCola,
        actualizar_salidas: els.repairUpdateOutputs.checked,
        redescargar_archivos: els.repairRedownloadFiles.checked,
        reintentos: Number(els.repairRetries.value || 2),
      }),
    });
    renderRepair(result);
    showToast(reiniciarCola ? 'Nueva revisión iniciada.' : 'Reparación iniciada o reanudada.', 'ok');
    navTo('reparacion');
  } catch (err) {
    showToast(`No se pudo iniciar la reparación: ${err.message}`, 'error');
  } finally {
    await refreshAll(true);
  }
}
...
async function stopRepair() {
  ...
  try {
    ...
    showToast(`No se pudo detener: ${err.message}`, 'error');
  }
}
...
if (els.repairResumeBtn) els.repairResumeBtn.addEventListener('click', () => startRepair(false));
if (els.repairNewBtn) els.repairNewBtn.addEventListener('click', () => startRepair(true));
if (els.repairStopBtn) els.repairStopBtn.addEventListener('click', stopRepair);
...
if (els.repairLogBox) startLogStream('reparacion', els.repairLogBox, els.repairLogSource, els.repairAutoscroll);
