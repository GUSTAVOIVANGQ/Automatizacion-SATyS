...
- systemd ejecuta el monitor diario a las 10:00 o a la hora configurada.
...
app = FastAPI(title="SATyS CRT", version="1.2.0")
...
def _get_config() -> dict[str, Any]:
    ...
    cfg.setdefault("timer_hora", os.getenv("SATYS_TIMER_HORA", "10:00"))
...
def _timer_unit_text(hora: str) -> str:
    return f"""[Unit]
Description=Ejecuta SATyS CRT diario a las {hora}
...
def log_ultimo(tail: int = Query(default=300, ge=1, le=5000), tipo: str = Query(default="diario")):
    log_path = _latest_manual_log_path() if tipo == "manual" else _latest_daily_log_path()
    if not log_path:
...
def iniciar_proceso_manual_diario():
    ...
    try:
        r = subprocess.run(["systemctl", "start", SERVICE_NAME], capture_output=True, text=True, timeout=20)
    except Exception as exc:
...
async def _stream_log(kind: str):
    ...
    while True:
        log_path = _latest_manual_log_path() if kind == "manual" else _latest_daily_log_path()
        if not log_path:
...
async def log_stream(tipo: str = Query(default="diario")):
    kind = "manual" if tipo == "manual" else "diario"
