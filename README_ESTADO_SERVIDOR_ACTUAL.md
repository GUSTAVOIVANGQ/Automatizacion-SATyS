...
La hora del servidor fue validada con `timedatectl` y está correcta para ejecutar la automatización diaria a las 01:00 AM hora de México.
...
# Rutas de datos: config/configuracion_local.json
# Rutas de datos: config/configuracion_local.json
# Rutas de datos: config/configuracion_local.json
# Rutas de datos: config/configuracion_local.json
# Rutas de datos: config/configuracion_local.json
...
```text
Tue 2026-07-14 01:00:00 CST ... satys-diario.timer → satys-diario.service
```

Esto confirma que el timer diario quedó programado para las 01:00 AM.
...
```ini
[Unit]
Description=SATyS CRT - ejecutar diario a las 01:00 AM
...
automatizar_registros_diario.py
  ...
  → sobrescribe TrámitesCRT.xlsx y copia por merge output/ y descargas/ en /depi/DEI_DATOS/SATyS
```
...
Archivo esperado dentro del proyecto:

```text
/data/gustavo.garcia/satys/Automatizacion-SATyS/config/configuracion_local.json
```

Contiene las credenciales SATyS, el App Password de Gmail, destinatarios, rutas Linux y concurrencia. Debe permanecer fuera de Git y con permisos:

```bash
chmod 600 /data/gustavo.garcia/satys/Automatizacion-SATyS/config/configuracion_local.json
```

Las contraseñas actuales no se rotaron; esto queda registrado como riesgo aceptado. El archivo no se sincroniza a CRT Recurso DEPI.
...
# Las rutas, credenciales y parámetros 10/900/2/2 se leen de:
...
Para producción, el servicio usa `--workers 10`.
...
- La automatización diaria está con `systemd timer` a las 01:00 AM.
