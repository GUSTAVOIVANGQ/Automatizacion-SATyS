...
Esta versión usa configuración hardcodeada dentro del archivo, como el
programa original. No depende de variables de entorno ni de archivos JSON de
...
# ══════════════════════════════════════════════════════════════════════
# CONFIGURACIÓN HARDCODEADA DE CORREO
# Edita aquí si cambian remitente, contraseña o destinatarios.
# ══════════════════════════════════════════════════════════════════════
...
EMAIL_FROM_NAME = "Sistema SATyS DEI"

DESTINATARIOS = [
    "gustavo.garcia@crt.gob.mx",
    "david.palestina@crt.gob.mx",
...
CARPETA_SALIDA = r"/depi/DEI_DATOS/SATyS"

# Adjuntar siempre el Excel principal al correo si existe y no supera el límite.
...
def _candidatos_excel_tramites(
    *,
    ...
    Orden de prioridad:
      ...
      2. Variable SATYS_EXCEL_PATH del servicio systemd.
      3. excel_path relativo/absoluto recibido por main_procesar.py.
      4. Carpeta compartida /depi/DEI_DATOS/SATyS/TrámitesCRT.xlsx.
      5. Excel local del proyecto.
    """
    ...
    add(os.environ.get("SATYS_EXCEL_PATH"))
    add(excel_path)
    ...
    <tr><td align="center">
      <table width="960" cellpadding="0" cellspacing="0" style="max-width:960px;background:#ffffff;border-radius:18px;overflow:hidden;box-shadow:0 10px 30px rgba(15,23,42,.14);">
        ...
        <tr><td style="background:#0f172a;color:#94a3b8;text-align:center;padding:20px 40px;font-size:12px;">
          Correo generado automáticamente por SATyS. Configuración de correo hardcodeada en notificar_email.py.
        </td></tr>
      </table>
    </td></tr>
  </table>
...
def enviar_notificacion(total_registros: int | None = None,
    ...
    Por defecto usa la configuración hardcodeada de este archivo.
    """
    ...
    if not habilitado:
        print("ℹ️  Notificación por correo deshabilitada por configuración hardcodeada.")
        return False

    tos = _split_list(destinatarios) if destinatarios is not None else list(DESTINATARIOS)
    if not tos:
        print("⚠️  No hay destinatarios hardcodeados en DESTINATARIOS.")
        return False
...
def enviar_desde_log_json(log_json_path: str | Path, destinatarios: list[str] | str | None = None) -> bool:
    """Lee procesamiento_log*.json y envía el correo usando los destinatarios hardcodeados por defecto."""
...
if __name__ == "__main__":
    ...
    ap = argparse.ArgumentParser(description="Notificación correo SATyS con configuración hardcodeada")
